...
<LayoutTop>
    ...
    <div class="bg-linear-to-br from-pink-50 to-pink-100 dark:from-pink-950 dark:to-pink-900
        ...
        <div class="w-full max-w-sm mx-auto">
            <div class="flex flex-col gap-4 sm:gap-6">
                ...
                <div class="flex flex-col items-center gap-2 text-center px-2">
                    <div class="flex size-10 sm:size-12 items-center justify-center rounded-lg
                    ...
                    <!-- Device status badge -->
                    {#if deviceName}
                        <div class="mt-2 w-full rounded-lg border px-3 py-2
                                {isApproved
                                    ? 'bg-pink-100 dark:bg-pink-900/30 border-pink-300 dark:border-pink-700'
                                    : 'bg-red-50 dark:bg-red-900/20 border-red-300 dark:border-red-800'}">
                            <div class="flex items-center justify-between gap-2">
                                <div class="text-left">
                                    <p class="text-xs font-semibold text-pink-900 dark:text-pink-100">
                                        {deviceName}
                                        <span class="font-light text-pink-500">({deviceId})</span>
                                    </p>
                                    <p class="text-xs mt-0.5
                                          {isApproved
                                              ? 'text-green-600 dark:text-green-400'
                                              : 'text-red-600 dark:text-red-400'}">
                                        {isApproved ? '✓ Approved' : '✗ Belum disetujui admin'}
                                    </p>
                                </div>
                                {#if !isApproved}
                                    <ShieldOffIcon class="size-5 shrink-0 text-red-400" />
                                {/if}
                            </div>
                        </div>
                    {/if}
                ...
                {#if deviceName && !isApproved}
                    <div class="mx-2 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200
                        ...
                        <p class="mt-1 text-xs text-red-500 dark:text-red-500">
                            Hubungi admin untuk mengaktifkan device ini sebelum dapat login
                        </p>
                    </div>
                {/if}
