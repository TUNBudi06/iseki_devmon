<script lang="ts">
    ...
    function handleLogin(e: SubmitEvent) {
        e.preventDefault();
        console.log('Admin login attempt:', { username: form.username, password: form.password });
        form.post(routeUrl(loginPost()),{
            onSuccess: async (params) => {
                console.log('Admin login successful',params);
                toast.success('Login berhasil! Selamat datang kembali, Admin.', {
                    description: 'Anda telah berhasil masuk ke panel administrasi.',
                    duration: 4000,
                });
            ...
            <div class="flex flex-col gap-4 sm:gap-6">
                ...
                <div class="flex flex-col items-center gap-2 text-center px-2">
                    <div class="flex size-10 sm:size-12 items-center justify-center rounded-lg
                        ...
                        <UserIcon class="size-5 sm:size-6 text-pink-600 dark:text-pink-400" />
                    </div>
                ...
                <form onsubmit={handleLogin} class="px-2">
                    <FieldGroup>
                        <Field>
                            <FieldLabel for="email" class="text-sm font-medium">Username</FieldLabel>
                            <Input
                                id="email"
                                type="text"
