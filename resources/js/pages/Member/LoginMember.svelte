<script lang="ts">
    ...
    let blocked = $derived(
        !device || !device.approved || !device.registered || !!serverError,
    );
    let deviceId = $derived(device?.model_id ?? '');

    let title = $derived(
        mode === 'primary' ? 'Absensi Perangkat' : 'Absensi Tambahan',
    );
    let description = $derived(
        mode === 'primary'
            ? today
            : `Input absen untuk pengguna lain — ${today}`,
    );
    let backRoute = $derived(
        mode === 'primary' ? home().url : dashboard(deviceId).url,
    );
    let backLabel = $derived(
        mode === 'primary' ? 'Kembali' : 'Kembali ke Dashboard',
    );
    ...
    function handleSubmit(e: Event) {
        ...
        http.post(loginMember.store(deviceId).url, {
            onSuccess: () => {
                toast.success(
                    'Absensi berhasil! Terima kasih telah melakukan absensi hari ini.',
                );
                setTimeout(() => {
            ...
            onError: (errors) => {
                localError =
                    errors.error || errors.message || 'Terjadi kesalahan';
            },
        });
    }
            ...
            <Card.Content class="space-y-4">
                    ...
                    <div class="rounded-xl bg-muted/30 p-3 space-y-1.5">
                        <div class="flex items-center gap-2 justify-center">
                            ...
                            <span class="font-semibold text-sm"
                                >{device.model_name}</span
                            >
                        </div>
                        {#if device.imei}
                            <div class="text-center">
                                <span
                                    class="text-[10px] text-primary/70 uppercase tracking-wider"
                                    >IMEI</span
                                >
                                <code
                                    class="text-xs font-mono font-bold text-foreground"
                                    >{device.imei}</code
                                >
                            </div>
                ...
                <form onsubmit={handleSubmit} class="space-y-4">
                    ...
                    <Button
                        type="submit"
                        class="w-full gap-2"
                        disabled={http.processing || blocked}
                    >
                        {#if http.processing}
                            <div
