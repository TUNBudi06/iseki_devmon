<script lang="ts">
    ...
    let { brands, nextIncrement }: { brands: Brand[]; nextIncrement: string } =
        $props();
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            if (!res.ok || !data.success) {
                const msg =
                    ...
                    (data.errors
                        ...
                        : null) ??
                    'Gagal mengajukan perangkat (HTTP ' + res.status + ')';
                toast.error(msg);
                isSubmitting = false;
                return;
            }
            ...
            toast.success(
                'Perangkat berhasil didaftarkan! Menunggu persetujuan admin.',
            );
            setTimeout(
                () => router.visit(loginMember(data.device_id).url),
                1500,
            );
        } catch (err) {
...
<LayoutBG
    class="min-h-screen bg-background flex flex-col items-center justify-center p-4 md:p-6 relative overflow-hidden"
>
    <Particles
        particleCount={200}
    ...
    <div class="w-full max-w-lg relative z-10">
        <button
            ...
            class="mb-4 flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors"
        >
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <Card.Title class="text-lg"
                            >Daftarkan Perangkat Manual</Card.Title
                        >
                        <Card.Description
                            >Data akan ditinjau admin sebelum dapat digunakan</Card.Description
                        >
                    </div>
                </div>
                <Badge
                    variant="outline"
                    class="mt-3 bg-amber-500/10 border-amber-300/30 text-amber-500 text-xs"
                    >Menunggu Persetujuan Admin</Badge
                >
            </Card.Header>
            ...
            <Card.Content class="space-y-4">
                ...
                <div class="space-y-2">
                    ...
                    <select
                        ...
                        class="flex h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                    >
                    ...
                    {#if form.errors.brand_id}<p class="text-xs text-rose-400">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <Label for="model_id" class="text-sm font-medium"
                            >Model ID</Label
                        >
                        <Input
                            id="model_id"
                            bind:value={form.model_id}
                            oninput={onModelIdInput}
                            placeholder="Otomatis terisi..."
                            class="h-10"
                        />
                        {#if form.errors.model_id}<p
                    ...
                    <div class="space-y-2">
                        <Label for="model_name" class="text-sm font-medium"
                            >Model Name</Label
                        >
                        <Input
                            id="model_name"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <Label for="model_type" class="text-sm font-medium"
                            >Tipe</Label
                        >
                        <select
                            id="model_type"
                            bind:value={form.model_type}
                            class="flex h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                        >
                    ...
                    <div class="space-y-2">
                        <Label for="buy_date" class="text-sm font-medium"
                            >Tanggal Pembelian</Label
                        >
                        <Input
                            id="buy_date"
                            bind:value={form.buy_date}
                            placeholder="2025-01-15"
                            class="h-10"
                        />
                        {#if form.errors.buy_date}<p
                                class="text-xs text-rose-400"
                            >
                                {form.errors.buy_date}
                            </p>{/if}
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        ...
                        <Input
                            ...
                            class="h-10"
                        />
                        {#if form.errors.ram}<p class="text-xs text-rose-400">
                                {form.errors.ram}
                            </p>{/if}
                    </div>
                    <div class="space-y-2">
                        <Label for="storage" class="text-sm font-medium"
                            >Storage</Label
                        >
                        <Input
                            id="storage"
                ...
                <div class="space-y-2">
                    ...
                    <Input
                        ...
                        class="h-10"
                    />
                    {#if form.errors.price}<p class="text-xs text-rose-400">
                            {form.errors.price}
                        </p>{/if}
                </div>
                ...
                <div>
                    ...
                    <p class="text-xs text-muted-foreground mb-3">
                        Isi minimal salah satu (IMEI untuk HP, MAC Address untuk
                        tablet/hp). Lihat di <b>Settings &gt; About Phone</b>.
                    </p>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <Input
                                ...
                                placeholder="352678094561230"
                                maxlength={15}
                                class="h-10 font-mono"
                            />
                            {#if form.errors.imei}<p
                                    class="text-xs text-rose-400"
                                >
                                    {form.errors.imei}
                                </p>{/if}
