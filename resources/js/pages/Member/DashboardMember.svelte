<script lang="ts">
    ...
    let {
        ...
        deviceLatest: UserAbsence | null;
        currentDevice: {
            model_id: string;
            model_name: string;
            hash_token?: string | null;
        };
        hasAbsence: DeviceItem[];
        noAbsence: DeviceItem[];
    } = $props();

    const tableHasAbsence = $derived(
        hasAbsence.length > 0
            ? new TableHandler(hasAbsence, { rowsPerPage: 10 })
            : null,
    );
    const tableNoAbsence = $derived(
        noAbsence.length > 0
            ? new TableHandler(noAbsence, { rowsPerPage: 10 })
            : null,
    );
    ...
    let catatanInput = $state(
        sessionStorage.getItem(STORAGE_KEY) ?? deviceLatest?.catatan ?? '',
    );
    ...
    async function saveCatatan() {
        ...
        savingCatatan = true;
        router.post(
            dashboard.catatan(deviceId).url,
            {
...
<LayoutBG class="min-h-screen bg-background p-4">
    ...
    <div class="w-full max-w-3xl mx-auto relative z-10 space-y-4">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="text-right">
                <Badge variant="secondary" class="text-xs"
                    >{hasAbsence.length + noAbsence.length} total device</Badge
                >
                <div class="text-xs text-muted-foreground font-mono mt-1">
                    {currentDevice.model_id}
                </div>
            </div>
        </div>
        ...
        {#if deviceLatest}
            <Card.Root
                class="border-emerald-500/30 bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 backdrop-blur-xl overflow-hidden"
            >
                <div
                    class="absolute top-0 right-0 size-32 bg-emerald-500/10 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl"
                ></div>
                ...
                <Card.Content class="p-4 relative">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <span
                            class="font-semibold text-emerald-600 dark:text-emerald-400 text-sm"
                            >Terakhir Absen</span
                        >
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            >
                            <span class="font-medium font-mono"
                                >{deviceLatest.time_absence}</span
                            >
                        </div>
                    </div>
                    ...
                    <!-- Catatan -->
                    <div
                        class="mt-4 pt-4 border-t border-emerald-500/20 space-y-2"
                    >
                        <div
                        ...
                        >
                            ...
                            <span
                                class="font-medium text-emerald-600 dark:text-emerald-400 text-xs uppercase tracking-wider"
                                >Catatan</span
                            >
                        </div>
                        <Textarea
        ...
        {:else}
            <Card.Root
                class="border-amber-500/30 bg-gradient-to-br from-amber-500/10 to-amber-500/5 backdrop-blur-xl overflow-hidden"
            >
                <Card.Content class="p-4 relative">
                    <div class="flex items-center gap-2">
                        <Clock class="size-5 text-amber-500" />
                        <span
                            class="font-semibold text-amber-600 dark:text-amber-400 text-sm"
                            >Belum Ada Absen Hari Ini</span
                        >
                    </div>
                    <p class="text-sm text-muted-foreground mt-1">
                        Gunakan perangkat ini untuk melakukan absensi
                    </p>
                </Card.Content>
            </Card.Root>
        {/if}
        ...
        <div class="flex justify-center gap-3 py-2 flex-wrap">
            ...
            {/if}
            <Button
                onclick={() => router.visit(loginMember.input(deviceId).url)}
                class="gap-2"
                size="lg"
            >
        ...
        <Card.Root class="border-border/60 bg-card/80 backdrop-blur-xl">
            <Card.Header class="pb-3">
                <Card.Title class="text-base flex items-center gap-2">
                    ...
                    <Badge variant="secondary" class="ml-auto text-xs"
                        >{hasAbsence.length} device</Badge
                    >
                </Card.Title>
            </Card.Header>
            ...
            <Card.Content>
                ...
                {:else if tableHasAbsence}
                    <div
                        class="flex items-center gap-3 px-4 py-2 border-b border-border/30 bg-muted/10 flex-wrap"
                    >
                    ...
                    <div class="p-3 space-y-2">
                        {#each tableHasAbsence.rows as row}
                            <div
                                class="rounded-xl border border-border/60 bg-card p-3 flex items-center justify-between gap-3"
                            >
                                    ...
                                    {#if row.latest_user_name}
                                        ...
                                        >
                                            ...
                                            <span class="font-medium"
                                                >{row.latest_user_name}</span
                                            >
                                            <span
                                                class="text-muted-foreground font-mono"
                                                >· {row.latest_user_nik}</span
                                            >
                                            <span class="text-muted-foreground"
                                                >· {row.latest_time}</span
                                            >
                                        </div>
                                    {:else}
                                        <div
                                ...
                                <Badge
                                    class="bg-emerald-500/15 text-emerald-600 border-emerald-300/30 text-xs whitespace-nowrap shrink-0"
                                >
                    ...
                    <div class="p-3 space-y-2">
                        {#each tableNoAbsence.rows as row}
                            ...
                            >
                                ...
                                <Badge
                                    variant="outline"
                                    class="text-xs text-muted-foreground border-dashed whitespace-nowrap shrink-0"
                                >
...
{#if showQrModal && currentDevice.hash_token}
    ...
    >
        <Card.Root
            class="w-full max-w-sm border-violet-500/30 bg-card shadow-2xl animate-in zoom-in-95 duration-200"
        ...
        >
                ...
                <div class="flex items-center justify-center gap-3 mb-1">
                    ...
                    <div class="text-left">
                        <Card.Title class="text-lg"
                            >Verifikasi Perangkat</Card.Title
                        >
                        <Card.Description>
            ...
            <Card.Content class="pt-4 pb-2 flex flex-col items-center gap-4">
                <div
                    class="rounded-2xl bg-white p-4 shadow-lg ring-1 ring-violet-500/20"
                >
                    <QRCode
                        value={currentDevice.model_id +
                            ';' +
                            currentDevice.hash_token}
                        size={220}
                        fgColor="#000000"
                        bgColor="#ffffff"
                    />
                ...
                <div class="text-center space-y-1">
                    <code
                        class="text-xs font-mono break-all text-violet-600 dark:text-violet-400"
                        >{currentDevice.model_id}</code
                    >
                </div>
            </Card.Content>
